/*  Kevin Trinh
    CSE 2
    lab10 - MethodCalls
    October 31, 2014
    
    This code is an exercise on methods calling other methods.
*/

// add class
public class MethodCalls{
    
    // add addDigit method
    public static int addDigit(int number, int digit) {
        
        if (digit >= 0 && digit <= 9 && number > 0) {
            
            String digitString  = Integer.toString(digit);
            String numberString  = Integer.toString(number);
            String addedDigitString = (digitString + numberString);
            int addedDigitInt = Integer.parseInt(addedDigitString);
            return addedDigitInt;
        
        }
        else if (digit >= 0 && digit <= 9 && number < 0) {
            
            String digitString  = Integer.toString(digit);
            String numberString  = Integer.toString(Math.abs(number));
            String addedDigitString = ("-" + digitString + numberString);
            int addedDigitInt = Integer.parseInt(addedDigitString);
            return addedDigitInt;
            
        }
        else {
            return number;
        }
        
    }
    
    // add join method
    public static int join(int digit, int number) {
        
        if (digit >= 0 && digit <= 9 && number > 0) {
            return addDigit(digit, number);
        }
        else if (digit >= 0 && digit <= 9 && number < 0) {
            return addDigit(digit, number);
        }
        else if (digit < 0 && number < 0) {
            String digitString  = Integer.toString(Math.abs(digit));
            String numberString  = Integer.toString(Math.abs(number));
            String addedDigitString = (digitString + numberString);
            int addedDigitInt = Integer.parseInt(addedDigitString);
            return addedDigitInt;
        }
        else {
            String digitString  = Integer.toString(digit);
            String numberString  = Integer.toString(number);
            String addedDigitString = (digitString + numberString);
            int addedDigitInt = Integer.parseInt(addedDigitString);
            return addedDigitInt;
        }
    }
    
    // add main method
    public static void main(String[] args) {
        
        int a=784,b=22,c;
        
        c=addDigit(a,3);
        
        System.out.println("Add 3 to "+a+" to get "+c);
        c=addDigit(addDigit(c,4),5);
        
        System.out.println("Add 3, 4, and 5 to "+a+" to get "+c);
        System.out.println("Add 3 to -98 to get: "+addDigit(-98,3));
       
       
        c=join(a,b);
        
        System.out.println("Join "+a+" to "+b+" to get "+c);
        System.out.println("Join 87, 42, and 83 to get "+join(87, join(42,83)));
        System.out.println("Join -9 and -90 to get "+join(-9,-90));
        
    }
}  